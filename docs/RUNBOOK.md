...
| What                              | How                                                                                                   |
| --------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Any Infisical-held secret         | Change it in the Infisical UI, then `make deploy-platform`. In-cluster `InfisicalSecret`s resync every 5m (`spec.syncConfig.resyncInterval`); `kubectl delete pod -n platform-secrets -l control-plane=controller-manager` forces it now. Note the resync only updates the **Secret** — a consumer that reads it as `env` still needs a rollout restart. |
| `CLOUDFLARE_TUNNEL_TOKEN`         | New connector token in Zero Trust → store at `/jterrazz-infrastructure` → Secret updates within 5m, then `kubectl rollout restart deploy/cloudflared -n platform-networking` (it reads `TUNNEL_TOKEN` once, at startup). |
