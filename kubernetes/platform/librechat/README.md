...
- **Private-only for now**: `ALLOW_REGISTRATION: "false"` plus the
  ...
  from `platform.yaml` and leaning on LibreChat's own auth.
...
`kubernetes/infrastructure/base/platform-namespaces.yaml`.
...
Both volumes come from the **platform chart's named-volume storage map**
(chart 2.0) declared in `platform.yaml` — `uploads` used to be a hand-written
...
2. **`gateway-netpol.yaml` — an additive NetworkPolicy.** The gateway is an
